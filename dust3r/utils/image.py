...
def load_images(folder_or_list, size, square_ok=False, verbose=True, dynamic_mask_root=None, crop=False, fps=0, num_frames=110, imgs=None):
    ...
    R = reg[:3, :3].clone()
    T = reg[:3, 3].clone()
    focal = torch.tensor(K[0, 0]).unsqueeze(-1)
    ...
    for path in folder_content:
        ...
        if path.lower().endswith(supported_images_extensions):
            ...
            if os.path.exists(dynamic_mask_path):
                ...
                dynamic_mask = dynamic_mask.resize([size, size], PIL.Image.BILINEAR)
