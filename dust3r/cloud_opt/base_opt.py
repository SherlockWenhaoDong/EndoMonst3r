...
class BasePCOptimizer (nn.Module):
    ...
    def _init_from_views(self, view1, view2, pred1, pred2,
        ...
        self.POSE_DIM = 7
        self.pw_poses = torch.zeros((self.n_edges, 4, 4))  # pairwise poses
        ...
        self.focals = None
        if 'focal' in view1 and 'focal' in view2:
            focals = [torch.zeros(1) for _ in range(self.n_imgs)]
            for v in range(len(self.edges)):
                idx = view1['idx'][v]
                focals[idx] = view1['focal'][v]
                idx = view2['idx'][v]
                focals[idx] = view2['focal'][v]
            self.focals = focals
        self.pps = None
        if 'pp' in view1 and 'pp' in view2:
            pps = [torch.zeros(2) for _ in range(self.n_imgs)]
            for v in range(len(self.edges)):
                idx = view1['idx'][v]
                pps[idx] = view1['pp'][v]
                idx = view2['idx'][v]
    ...
    def clean_pointcloud(self, **kw):
        ...
        all_pts3d = self.get_pts3d(raw=True)
    ...
    def compute_global_alignment(self, init=None, save_score_path=None, save_score_only=False, niter_PnP=10, **kw):
        ...
        self.pw_poses.requires_grad = False
...
def clean_pointcloud(im_confs, K, cams, depthmaps, all_pts3d,
    ...
    """
    K = K.unsqueeze(0).expand(len(depthmaps), -1, -1).to(all_pts3d.device)
    depthmaps = depthmaps.reshape(-1, 512, 512)
    all_pts3d = all_pts3d.reshape(-1, 512, 512, 3)
    cams = cams.expand(len(depthmaps), -1, -1).to(all_pts3d.device)
