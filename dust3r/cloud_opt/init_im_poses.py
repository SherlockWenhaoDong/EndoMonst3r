...
def init_from_known_poses(self, niter_PnP=10, min_conf_thr=3):
    ...
    image_size = xmlroot.find("param[@name='imageSize']").text.split()
    W, H = 512, 512

    K = list(map(float, xmlroot.find("param[@name='K']").text.split()))
    K = torch.tensor(K, dtype=torch.float32).reshape(3, 3)

    distortion = list(map(float, xmlroot.find("param[@name='distortion']").text.split()))
    distortion = torch.tensor(distortion, dtype=torch.float32)

    reg = list(map(float, xmlroot.find("param[@name='registration']").text.split()))
    reg = torch.tensor(reg, dtype=torch.float32).reshape(4, 4)
    self.pw_poses = reg.unsqueeze(0).expand(self.n_imgs, -1, -1)
    ...
    for e, (i, j) in enumerate(tqdm(self.edges, disable=not self.verbose)):
        ...
        # Use known camera poses directly
        P1 = reg.to(device)  # 4x4 homogeneous matrix
        P2 = reg.to(device)  # 4x4 homogeneous matrix

        # Compute relative transformation between P1 and P2
        # s, R, T are scale, rotation, translation to align the two poses
        s, R, T = align_multiple_poses(torch.stack((P1, P2)), torch.stack((P1, P2)))

        # Set pairwise pose using the known poses (scale may be 1.0 if fixed)
        # self._set_pose(self.pw_poses, e, R, T, scale=s)

        # Record score for depthmap quality
...
def minimum_spanning_tree(imshapes, edges, pred_i, pred_j, conf_i, conf_j, im_conf, min_conf_thr,
    ...
    n_imgs = max(max(i, j) for i, j in edges) + 1
