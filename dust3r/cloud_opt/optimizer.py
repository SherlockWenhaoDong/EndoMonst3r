...
class PointCloudOptimizer(BasePCOptimizer):
    ...
    def __init__(self, *args, optimize_pp=False, focal_break=20, shared_focal=False, flow_loss_fn='smooth_l1', flow_loss_weight=0.0, 
        ...
        self.has_im_poses = False  # by definition of this class
        ...
        tree = ET.parse("/home/dongwenhao/SurgicalRecon/2_NON_RIGID_DEFORMATION_PULL/Xi_Left.xml")
        root = tree.getroot()

        image_size = root.find("param[@name='imageSize']").text.split()
        W, H = 512, 512

        K = list(map(float, root.find("param[@name='K']").text.split()))
        K = torch.tensor(K, dtype=torch.float32).reshape(3, 3)

        self.K = K

        distortion = list(map(float, root.find("param[@name='distortion']").text.split()))
        distortion = torch.tensor(distortion, dtype=torch.float32)

        reg = list(map(float, root.find("param[@name='registration']").text.split()))
        reg = torch.tensor(reg, dtype=torch.float32).reshape(4, 4)

        self.imshapes = [(512, 512)]

        self.im_depthmaps = nn.ParameterList(torch.randn(H, W) / 10 - 3 for H, W in self.imshapes)
        self.reg = reg

        self.im_poses = reg
        self.shared_focal = True
        # self.im_focals = [torch.tensor([K[0, 0]], dtype=torch.float32)]
        # self.im_pp = [torch.tensor([K[0, 2], K[1, 2]], dtype=torch.float32)]

        self.im_focals = torch.ones((len(self.im_depthmaps), 1, 1), device=self.device) * K[0, 0]
        self.im_pp = torch.ones((len(self.im_depthmaps), 2), device=self.device) * torch.tensor([K[0, 2], K[1, 2]], device=self.device)

        for pp in self.im_pp:
        ...
        if self.batchify:
            n_imgs = max(max(i, j) for i, j in self.edges) + 1  # Determine number of images from edges
            self.im_areas = [H * W] * n_imgs  # [262144, 262144, ...]
            self.max_area = H * W
            self.im_depthmaps = nn.Parameter(torch.randn(n_imgs, self.max_area) / 10 - 3.0)

            self.im_poses = torch.stack([self.im_poses[0].clone() for _ in range(n_imgs)]).to(self.device)
            self.im_focals = torch.tensor([self.im_focals[0] for _ in range(n_imgs)], device=self.device)
            self.im_pp = torch.stack([self.im_pp[0].clone() for _ in range(n_imgs)]).to(self.device)

            # Pixel grid
            self.register_buffer('_pp', torch.tensor([(W / 2, H / 2)] * n_imgs, device=self.device))
            self.register_buffer('_grid', ParameterStack(
                [xy_grid(W, H, device=self.device) for _ in range(n_imgs)],
                is_param=False, fill=self.max_area
            ))

            # -----------------------------
            # Precompute pixel weights based on edges
            # -----------------------------
            ...
            self.register_buffer('_stacked_pred_i', ParameterStack(
                [self.pred_i[i_j] for i_j in self.str_edges],
                is_param=False, fill=self.max_area
            ))
            self.register_buffer('_stacked_pred_j', ParameterStack(
                [self.pred_j[i_j] for i_j in self.str_edges],
                is_param=False, fill=self.max_area
            ))
        # adding thing to window wise optimization
        elif self.window_wise:
            n_imgs = max(max(i, j) for i, j in self.edges) + 1  # number of images
            self.n_imgs = n_imgs

            # Image areas and depthmaps
            self.im_areas = [H * W] * n_imgs
            self.max_area = H * W
            self.im_depthmaps = nn.Parameter(torch.randn(n_imgs, H, W) / 10 - 3.0)

            # Duplicate camera parameters for all images
            self.im_poses = torch.stack([self.im_poses[0].clone() for _ in range(n_imgs)]).to(
                self.device)  # n_imgs x 4 x 4
            self.im_focals = torch.tensor([self.im_focals[0] for _ in range(n_imgs)], device=self.device)  # n_imgs
            self.im_pp = torch.stack([self.im_pp[0].clone() for _ in range(n_imgs)]).to(self.device)  # n_imgs x 2

            # Pixel grid for each image
            self.register_buffer('_pp', torch.tensor([(W / 2, H / 2)] * n_imgs, device=self.device))  # principal points
            self.register_buffer('_grid', ParameterStack(
                [xy_grid(W, H, device=self.device) for _ in range(n_imgs)],
                is_param=False, fill=self.max_area
            ))
            ...
            # Precompute stacked predictions
            self.register_buffer('_stacked_pred_i', ParameterStack(
                [self.pred_i[i_j] for i_j in self.str_edges],
                is_param=False, fill=self.max_area
            ))
        ...
        # -----------------------------
        # Total pixel areas for weighting
        # -----------------------------
    ...
    def load_window_params(self, window_idx):
        ...
        if window_idx == 0 and self.prev_video_results is None:
            ...
            self.opt_im_poses = self.im_poses[win_start:win_end]
            self.opt_im_focals = self.im_focals[win_start:win_end]
        else:
        ...
        self.register_buffer('_win_grid', ParameterStack(
            [xy_grid(W, H, device=self.device) for H, W in self.win_imshapes], fill=0))
    ...
    def save_window_params(self, window_idx):
        ...
        if window_idx == 0 and self.prev_video_results is None:
            for idx, i in enumerate(range(win_start, win_end)):
                ...
                depth_param.data[:] = self.opt_im_depthmaps[idx].view(self.imshapes[0][0], self.imshapes[0][1])
    ...
    def get_flow(self, sintel_ckpt=False): #TODO: test with gt flow
        ...
        args = argparse.Namespace(
            ckpt='/home/dongwenhao/SurgicalRecon/Models/raftstereo-sceneflow.pth',
            hidden_dims=[128] * 3,
        ...
        flow_net = model.module
        flow_net.to(device)
        flow_net.eval()

        # flow_net = load_RAFT() if sintel_ckpt else load_RAFT("/home/dongwenhao/SurgicalRecon/Models/raft-things.pth")
    ...
    def _set_depthmap_non_batch(self, idx, depth, force=False):
        param = self.im_depthmaps[idx].reshape(512, 512)
        if param.requires_grad or force:  # can only init a parameter not already initialized
            param.data[:] = depth.log().nan_to_num(neginf=0)
        return param
    ...
    def get_depthmaps(self, raw=True):
        if self.batchify:
    ...
    def depth_to_pts3d(self):
        ...
        im_poses = torch.stack(self.camera_poses).to(rel_ptmaps.device)
    ...
    def depth_to_win_pts3d(self):
        ...
        # Get depths and  projection params if not provided
        focals = self.im_focals[:self.window_size]
        pp = self.im_pp[:self.window_size]
        depth = self.get_win_depthmaps(raw=True)

        # get pointmaps in camera frame
        _win_grid = self._win_grid.unsqueeze(0).expand(depth.shape[0], -1, -1, -1)
        _win_grid = _win_grid.view(_win_grid.shape[0], -1, 2)
        rel_ptmaps = _fast_depthmap_to_pts3d(depth, _win_grid, focals, pp=pp)
        im_poses = self.reg.unsqueeze(0).expand(self.window_size, *self.reg.shape).to(rel_ptmaps.device)
        # project to world frame
        return geotrf(im_poses, rel_ptmaps)
    ...
    def get_pts3d(self, raw=True, **kwargs):
        if self.batchify:
    ...
    def forward_batchify(self, epoch=9999):
        # ============================================================
        # Fixed camera pose version (no pose optimization)
        # Only optimize depth maps, confidence, and per-pixel alignment
        # ============================================================

        # --- Use known (fixed) camera poses ---
        # Instead of using predicted or learned poses (self.get_pw_poses),
        # we directly take the camera poses from self.camera_poses.
        pw_poses = self.reg.unsqueeze(0).expand(self.n_imgs, 4, 4).to(self.device)  # (N, 4, 4), known poses
        proj_pts3d = self.get_pts3d(raw=True)  # reconstructed 3D points

        # Adaptation factor is disabled since poses are fixed
        pw_adapt = torch.ones((len(self.edges), 1, 1), device=self.device)

        # --- Transform the pairwise predictions to the same coordinate frame ---
        aligned_pred_i = geotrf(pw_poses[self._ei], pw_adapt * self._stacked_pred_i)
        aligned_pred_j = geotrf(pw_poses[self._ej], pw_adapt * self._stacked_pred_j)

        # --- Compute geometric alignment loss between predicted and projected 3D points ---
        li = self.dist(proj_pts3d[self._ei], aligned_pred_i, weight=self._weight_i).sum() / self.total_area_i
        lj = self.dist(proj_pts3d[self._ej], aligned_pred_j, weight=self._weight_j).sum() / self.total_area_j

        # --- Temporal smoothness (optional, only if you want temporal consistency) ---
        if self.temporal_smoothing_weight > 0:
            temporal_smoothing_loss = self.relative_pose_loss(
                torch.stack(self.camera_poses)[:-1],
                torch.stack(self.camera_poses)[1:]
            ).sum()
        else:
            temporal_smoothing_loss = 0

        # --- Optical flow consistency (optional) ---
        # If flow loss is enabled, compute the ego-motion–based flow between views.
        # Note: Since poses are fixed, this just evaluates consistency, not optimization.
        if self.flow_loss_weight > 0 and epoch >= self.num_total_iter * self.flow_loss_start_epoch:
            R_all, T_all = self.reg[:3, :3].to(self.device), self.reg[:3, 3:].to(self.device)
            R1, T1 = R_all, T_all
            R2, T2 = R_all, T_all
            K_all = self.K.to(self.device)
            inv_K_all = torch.linalg.inv(K_all)
            K_1, inv_K_1 = K_all, inv_K_all
            K_2, inv_K_2 = K_all, inv_K_all

            depth_all = torch.stack(self.get_depthmaps(raw=True)).unsqueeze(1)
            ...
            # If flow loss explodes (e.g., due to invalid dynamic masks), ignore it
        ...
        # Encourage current depth maps to stay close to the initial estimates.
        ...
        # --- Total loss ---
        # Combine geometric alignment, temporal smoothness, flow consistency, and depth regularization.
        loss = (li + lj) \
               + self.temporal_smoothing_weight * temporal_smoothing_loss \
               + self.flow_loss_weight * flow_loss \
               + self.depth_regularize_weight * depth_prior_loss

        return loss, flow_loss

    def forward_window_wise(self, epoch=9999):
        pw_poses = self.reg  # cam-to-world

        pw_adapt = self.get_win_adaptors().unsqueeze(1)
        proj_pts3d = self.get_win_pts3d(raw=True)

        pw_poses = pw_poses.unsqueeze(0).expand(len(proj_pts3d), 4, 4).to(self._win_stacked_pred_i.device)
...
def _fast_depthmap_to_pts3d(depth, pixel_grid, focal, pp):
    pp = pp.unsqueeze(1).to(depth.device)
    focal = focal.unsqueeze(1).unsqueeze(-1).to(depth.device)
    assert focal.shape == (len(depth), 1, 1)
    assert pp.shape == (len(depth), 1, 2)
    assert pixel_grid.shape[:2] == depth.shape, f"{pixel_grid.shape} vs {depth.shape}"
